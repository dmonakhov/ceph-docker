...
function osd_disk_prepare {
  ...
  if [[ ! -z "${OSD_JOURNAL}" ]]; then
    if [[ ${OSD_BLUESTORE} -eq 1 ]]; then
      ...
      umount /var/lib/ceph/osd-lockbox/$(blkid -o value -s PARTUUID ${OSD_DEVICE}3) || true
    else
    ...
    if [[ ${OSD_BLUESTORE} -eq 1 ]]; then
      ...
      # NOTE(leseb): adding || true so when this bug will be fixed the entrypoint will not fail
      # Ceph bug tracker: http://tracker.ceph.com/issues/18944
      umount /var/lib/ceph/osd-lockbox/$(blkid -t TYPE=crypto_LUKS -o value -s PARTUUID ${OSD_DEVICE}1) || true
    else
